        ...
        if ((left == 1 && gameData.charAt(0) == 'L') || (left == -1) && gameData.charAt(0) == 'R')
        {
            switch (state)
            {
                ...
                case driveArc2:
                    ...
                    drivePower = drivePID.pidCalculate(target, encoder);
                    /*
                     * if(!slewCut) { drivePower = slewRate.rateCalculate(drivePower); }
                     * if(Math.abs(drivePower)>=0.9) { slewCut = true; }
                     */
                    /*
                     * ellipseX = 20; ellipseY = 120; x = xEllipseCalculate(ellipseX, ellipseY,
                     * encoder); y = yEllipseCalculate(ellipseX, ellipseY, x); angle =
                     * -angleEllipseCalculate(ellipseX, ellipseY, x);
                     */
                    /*
