...
public class MoveLiftToPos extends Command {
    ...
    private SlewRate liftSlew;
    private static final double startPos = 0;
    private static final double carriage1 = Robot.lift.PLATTOP;
    private static final double top = Robot.lift.PLATTOP + Robot.lift.STAGETOP;
    ...
    private double stageSpeed = 20.64;
  ...
  //private Lift.direction direction = Lift.direction.UP;
    ...
    protected void execute() {
        ...
        if (platFirst)
        {
            ...
            {
                Robot.lift.movePlat(Robot.lift.platUpPID.pidCalculate(posPlatFinal, Robot.lift.getPlatEncoder()));
            }
            else
            {
                Robot.lift.stopPlat();
        ...
        else
        {
            ...
            {
                if (!accel && Math.abs(last)<0.6)
                {
                    double val = -Robot.lift.stageUpPID.pidCalculate(posStageFinal, Robot.lift.getStageEncoder());
                    val = liftSlew.rateCalculate(val);
                    accel = val > 0.6;
                    Robot.lift.moveStage(val);
                }
                else
                {
                    Robot.lift.moveStage(-Robot.lift.stageUpPID.pidCalculate(posStageFinal, Robot.lift.getStageEncoder()));
                }
            }
